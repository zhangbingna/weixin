...
# drive.find_element_by_id("start_date").send_keys(time.strftime("%Y%m%d")) # 获取当前时间并格式化
...
# drive.find_element_by_id("end_date").send_keys(time.strftime("2019-06-13")) # 获取当前日期time.strftime("%Y/%m/%d")
...
for j in range(int(a)):
    ...
    for i in range(100):
        try:
            bianji = drive.find_element_by_xpath(
                u"(.//*[normalize-space(text()) and normalize-space(.)='删除'])[" + str(i + 1) + "]/following::button[1]")
        except NoSuchElementException:
            break
        con = drive.find_element_by_xpath('// *[ @ id = "feedTable"]/tbody/tr[' + str(i + 1) + '] / td[6]')
        osCon = drive.find_element_by_xpath("//*[ @ id = 'feedTable']/tbody/tr[" + str(i + 1) + "]/td[3]")
            ...
            for k, v in dict.items():
                for a in v:
                    if a in con.text:
                        ...
                        print(content[k])
                        bianji.click()
                        s = drive.find_element_by_id("mySelect")
                        Select(s).select_by_value(str(content[k]))
                        #drive.find_element_by_id("mySelect").click()

                        #drive.find_element_by_id("mySelect").send_keys(str(content[k]))
                        bianji.click()
                        num2 = num2 + 1
                        raise KeyError
            ...
            for a in entry:
                if a in con.text:
                    ...
                    if osCon.text[0] == "a":
                        ...
                        if int(os[1]) >= 27:
                            bianji.click()
                            drive.find_element_by_id("mySelect").send_keys("欧盟用户找不到入口")
                            bianji.click()
                            print("['欧盟用户找不到入口1']")
                            num2 = num2 + 1
                        else:
                            bianji.click()
                            drive.find_element_by_id("mySelect").send_keys("看一看入口找不到")
                            bianji.click()
                            print("['看一看入口找不到1']")
                            num2 = num2 + 1
                    elif osCon.text[0] == "i":
    ...
    print("未编辑%d条" % num)
    print("---------------------------------------")
    drive.find_element_by_id("feedTable_next").click()
    x = x + num
    y = y + num2
print("总共编辑了%d条" % y)
print("剩余%d条未编辑" % x)
